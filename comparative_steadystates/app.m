...
ui <- fluidPage(withMathJax(),
                titlePanel(h1('Comparing steady states in the Solow model')),
                sidebarLayout(
                  sidebarPanel(
                    p(
...
server <- function(input, output) {
  ...
  output$solow_plot <- renderPlot({
    ...
    isolate({
      ...
      ggplot(data.frame(k = c(0, 1.1 * max(
        kstar1(), kstar2()
      )))) +
        stat_function(
          fun = function(k)
            isolate(input$delta1) * k,
          #  geom = "line",
        ...
        stat_function(
          fun = function(k)
            isolate(input$A1) * k ^ (isolate(input$alpha1)) * isolate(input$h1) ^ (1 - isolate(input$alpha1)),
          #    geom = "point",
        ...
        stat_function(
          fun = function(k)
            isolate(input$saverate1) * isolate(input$A1) * k ^ (isolate(input$alpha1)) * isolate(input$h1) ^ (1 - isolate(input$alpha1)),
        ...
        stat_function(
          fun = function(k)
            isolate(input$A2) * k ^ (input$alpha2) * isolate(input$h2) ^ (1 - input$alpha2),
        ...
        stat_function(
          fun = function(k)
            isolate(input$saverate2) * isolate(input$A2) * k ^ (input$alpha2) * isolate(input$h2) ^ (1 - input$alpha2),
        ...
        scale_x_continuous(expand = c(0, 0), limits = c(0, 2 * max(kstar1(), kstar2()))) +
